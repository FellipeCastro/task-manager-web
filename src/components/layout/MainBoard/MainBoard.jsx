...
const MainBoard = ({ activeBoard, handleTaskClick, loading }) => {
    ...
    if (!activeBoard) {
        return (
            <div className="main-board">
                <p className="msg">Selecione um painel</p>
            </div>
        );
    }
    ...
    const todoTasks = activeBoard.tasks.filter(
        (task) => countCompletedSubtasks(task.subtasks) === 0
    );
    ...
    const doneTasks = activeBoard.tasks.filter(
        (task) => countCompletedSubtasks(task.subtasks) === task.subtasks.length
    );
    ...
    return (
        <div className="main-board">
            <div className="column">
                ...
                {todoTasks.map((task) => {
                    return (
                        ...
                        >
                            <h4>{task.title}</h4>
                            <span>
                                {countCompletedSubtasks(task.subtasks)} de{" "}
                ...
                {todoTasks.length === 0 && (
                    <p className="msg">Não há tarefas a serem feitas</p>
                )}
            </div>
            <div className="column">
                <div className="title">
                    <span className="doing"></span>{" "}
                    <h3>Fazendo ({doingTasks.length})</h3>
                </div>
                ...
                {doingTasks.length === 0 && (
                    <p className="msg">Não há tarefas sendo feitas</p>
                )}
            </div>
            <div className="column">
                <div className="title">
                    <span className="done"></span>{" "}
                    <h3>Feito ({doneTasks.length})</h3>
                </div>
                ...
                {doneTasks.length === 0 && (
                    <p className="msg">Não há tarefas feitas</p>
                )}
            </div>
        </div>
    );
